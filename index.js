...
if (detectedMood !== user.mood) {
   ...
   await user.save();
    ...
    // Daily Limit
let dailyLimit = 5; // default free

if (user.plan === "pro") {
  dailyLimit = 100; // fair usage 100 interactions
}

if (user.plan === "premium") {
  dailyLimit = 100; // also 100 per day
}
...
app.post("/upgrade", authenticateToken, async (req, res) => {
  try {
    ...
    if (plan === "pro") {
      user.plan = "pro";
      user.planExpiry = new Date(Date.now() + 24 * 60 * 60 * 1000); // 24 hours
    ...
    res.json({ success: true, plan: user.plan, expiry: user.planExpiry });
